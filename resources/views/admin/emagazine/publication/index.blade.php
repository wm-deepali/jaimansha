...
<div class="container mt-4">
    ...
    @if($authorType === 'magazine' || $authorType === 'both')
        <tr>
            <!--<td>{{ $index + 1 }}</td>-->
            <td>{{ $pub->author->name ?? 'N/A' }}</td>
<td>{{ \Illuminate\Support\Str::limit($pub->title, 10) }}</td>
<td>{!! \Illuminate\Support\Str::limit(strip_tags($pub->short_description), 20) !!}</td>
...
<div class="modal fade" id="addPublicationModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog">
    <form action="{{ route('admin.emagazine.publication.store') }}" method="POST" class="modal-content">
        @csrf
    ...
    document.querySelectorAll('.editBtn').forEach(button => {
        button.addEventListener('click', function () {
         ...
         $('#edit-description').summernote('code', description || '');
            ...
            document.getElementById('editForm').action = `/admin/emagazine/publication/update/${id}`;
        });
    });
</script>
@endsection
